...
function resolveAnchorTarget(targetId: string | undefined, blocks: Block[]): string | null {
  ...
  const exact = candidates.find((candidate) => candidate.normalizedId === normalizedTarget);
  if (exact) {
    return exact.id;
  }

  for (const variant of targetVariants) {
    const prefixed = candidates.find((candidate) => candidate.normalizedId === variant || candidate.normalizedId.startsWith(`${variant}-`));
    if (prefixed) {
      return prefixed.id;
    }
  }
  ...
  for (const variant of targetVariants) {
    const labelMatch = candidates.find((candidate) => candidate.normalizedLabel === variant || candidate.normalizedLabel.includes(variant));
    if (labelMatch) {
...
export function validateGeneratedHtml(fullHtml: string): ValidationIssue[] {
  ...
  Array.from(documentFragment.querySelectorAll('[id]')).forEach((element) => {
  ...
  Array.from(documentFragment.querySelectorAll('section[data-block-id]')).forEach((section) => {
    const value = section.getAttribute('data-block-id')?.trim();
    if (!value) return;

    if (seenBlockIds.has(value)) {
      duplicateBlockIds.add(value);
      return;
    }
  ...
  Array.from(documentFragment.querySelectorAll('section')).forEach((section) => {
    ...
    if (isHeroSection(section) && usesViewportHeight(section) && !hasIntentionalVerticalBalancing(section)) {
      ...
      issues.push({
        type: 'warning',
        code: 'hero-balance',
        message: `Hero section "${sectionId}" uses full-viewport height without clear vertical balancing, which can create awkward empty space or top-heavy layouts.`,
        blockId: section.getAttribute('data-block-id') || undefined,
        targetId: section.getAttribute('id') || undefined,
      });
    }
    ...
    if (isSocialProofSection(section) && hasCrowdedSocialProofLayout(section)) {
      ...
      issues.push({
        ...
        message: `Social proof section "${sectionId}" uses a cramped desktop metrics grid. Prefer a wider 2-column proof grid or an explicitly sized proof column.`,
        blockId: section.getAttribute('data-block-id') || undefined,
        targetId: section.getAttribute('id') || undefined,
      });
    }
...
function ensureSectionBackground(html: string): string {
  if (/class="[^"]*\bbg-[^\s"]+/.test(html) || /style="[^"]*background/.test(html)) {
    return html;
  }
